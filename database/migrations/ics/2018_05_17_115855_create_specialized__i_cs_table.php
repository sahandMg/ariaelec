...
class CreateSpecializedICsTable extends Migration
{
    public $cols = [];
    /**
     * Run the migrations.
    ...
    public function up()
    {
        ...
        Schema::create('specialized__ics', function (Blueprint $table) {
            $table->increments('id');
            for($i=0;$i<count($this->cols);$i++){
                $table->string($this->cols[$i])->nullable();
            }
            $table->unsignedInteger('common_id')->default(101);
    ...
    {
        Schema::dropIfExists('specialized__ics');
    }
}
