...
class CreateClockTimingRealTimeClocksTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('clock_timing__real__time__clocks', function (Blueprint $table) {
            $table->increments('id');
            for($i=0;$i<count($this->cols);$i++){
                $table->string($this->cols[$i])->nullable();
            }
            $table->unsignedInteger('common_id')->default(8);
    ...
    {
        Schema::dropIfExists('clock_timing__real__time__clocks');
    }
}
