...
class CreatePMICPowerDistributionSwitchesLoadDriversTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('pmic__power__distribution__switches__load__drivers', function (Blueprint $table) {
            $table->increments('id');
            for($i=0;$i<count($this->cols);$i++){
                $table->string($this->cols[$i])->nullable();
            }
            $table->unsignedInteger('common_id')->default(86);
    ...
    {
        Schema::dropIfExists('pmic__power__distribution__switches__load__drivers');
    }
}
