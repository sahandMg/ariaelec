...
class CreateInterfaceUARTsUniversalAsynchronousReceiverTransmittersTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('interface__uarts__receiver__transmitters', function (Blueprint $table) {
            $table->increments('id');
            for($i=0;$i<count($this->cols);$i++){
                $table->string($this->cols[$i])->nullable();
            }
            $table->unsignedInteger('common_id')->default(43);
    ...
    {
        Schema::dropIfExists('interface__uarts__receiver__transmitters');
    }
}
