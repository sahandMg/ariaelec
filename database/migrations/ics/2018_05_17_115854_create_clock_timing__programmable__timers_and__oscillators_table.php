...
class CreateClockTimingProgrammableTimersAndOscillatorsTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('clock_timing__programmable__timers_and__oscillators', function (Blueprint $table) {
            $table->increments('id');
            for($i=0;$i<count($this->cols);$i++){
                $table->string($this->cols[$i])->nullable();
            }
            $table->unsignedInteger('common_id')->default(7);
    ...
    {
        Schema::dropIfExists('clock_timing__programmable__timers_and__oscillators');
    }
}
