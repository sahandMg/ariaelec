...
class CreateLogicGatesAndInvertersMultiFunctionConfigurablesTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('logic__gates_and__inverters__multi_function__configurables', function (Blueprint $table) {
            $table->increments('id');
            for($i=0;$i<count($this->cols);$i++){
                $table->string($this->cols[$i])->nullable();
            }
            $table->unsignedInteger('common_id')->default(58);
    ...
    {
        Schema::dropIfExists('logic__gates_and__inverters__multi_function__configurables');
    }
}
